...
def main():
    ...
    try:
        while cap.isOpened():
            ...
            left_landmarks_list, right_landmarks_list = get_left_right_landmarks(frame, processed, draw, mpHands)

            # for changing brightness
            if left_landmarks_list:
                left_distance =  get_distance(frame, left_landmarks_list)
                b_level = np.interp(left_distance, [50,220], [0,100])
                sbc.set_brightness(b_level)

            # for changing volume
            if right_landmarks_list:
                right_distance = get_distance(frame, right_landmarks_list)
...
def get_distance(frame, landmarks_list):
    ...
    (x1,y1), (x2,y2) = (landmarks_list[0][1], landmarks_list[0][2]), \
    (landmarks_list[1][1], landmarks_list[1][2])
